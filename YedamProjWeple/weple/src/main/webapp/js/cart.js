...
function cartList(result) {
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    console.log(rowDiv);
    console.log(rowDiv.querySelector("div.basketprice").childNodes);
    console.log(rowDiv.querySelector("div.sum").childNodes);
    rowDiv.setAttribute("data-id", result[i].no);
    rowDiv.querySelector("div.pname span").textContent = result[i].productNm;
    rowDiv.querySelector('div.basketprice input[name="p_price"]').value =
      result[i].price;
    rowDiv.querySelector("div.basketprice").childNodes[2].textContent =
      result[i].price;
    rowDiv.querySelector("div.num input").value = result[i].qty;
    rowDiv.querySelector("div.sum").value = result[i].price * result[i].qty;
...
function sendProdId(result) {
	let checkBox = document.querySelectorAll(".checkbox");
	for(let i = 0; i < checkBox.length; i++) {
		if(checkBox[i].checked) {
			selectedBox.push(result[i].productNm);
		}
	}
	// 문자열 형태로 배열 전달
	document.from.selectedBox.value=selectedBox.join(",");
...
let basket = {
  ...
  changePNum: function (pos) {
    ...
    //AJAX 업데이트 전송
    // 전송처리결과가 성공이면 reCalc, updateUI 실행
    fetch("../updateCart.do")
      .then(this.reCalc)
      .then(this.updateUI)
      .catch((err) => console.log(err));
  ...
  selectProdBuy: function() {
	location.href = "/buyForm.do?totalPrice=" + this.totalPrice;
	this.reCalc();
	this.updateUI();
	},
};
